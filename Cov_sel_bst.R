...
library(mlr)
parallelMap::parallelStartSocket(8)
...
### Boosted tree for variable importance
### Speed up the turning procefure  useing parallel computing
...
{ ### objective = "reg:linear")
  ...
  model_Params <- ParamHelpers::makeParamSet(
    # number of rounds
    ParamHelpers::makeIntegerParam("nrounds", lower = 50, upper = 200),
...
BTVS_perm_trad <- function(x, y, perms = 20,pctl = .95,
                           ...
                           objective_2="reg:linear")
{
  ...
  imps <- lapply(X = perm_dat_list, FUN = BST_vimp, y = y,objective_1=objective_1,objective_2=objective_2)
  imp <- imps[[1]]
...
### xg-boost variable selection usig XY algorithem
...
### xg-boost variable selection usig XT algrithem
#BT_sel_XT <- function(df,perms = 20, pctl = .95)
 # {
    ...
    perms=20
    pctl=0.95
    XT<-BTVS_perm_trad(x=X,y=T,perms = perms,pctl = pctl,objective_1="classif.xgboost",
    objective_2="reg:logistic")
